...
//Necrioss for the wicked car/stats save system and the login/r system and settings file
//[CBK]$$$PiMp Made this
//===============================Include Files==================================
#include <dini>
#include <dudb>
...
#include <float>
//===============================Class Defines==================================
#58.9197,796.3157,10.8203
#25.0327,1008.2620,10.8203
#80.5237,1679.5144,11.0576
#97.0854,2460.2087,10.8203
#88.6165,1990.5101,10.8203
#35.3155,2502.2046,16.4922
#
...
AddStaticVehicle(411,2066.5452,821.3041,7.0130,358.6087,12,1); //
AddStaticVehicle(411,2063.6008,821.4567,6.9853,357.5729,75,1); //
AddStaticVehicle(411,2060.8059,821.4467,6.9863,358.9319,123,1); //
AddStaticVehicle(411,2061.7568,821.1879,7.0236,357.4594,123,1); //
AddStaticVehicle(411,2057.6953,821.6580,6.9284,0.2859,106,1); //
AddStaticVehicle(411,2054.9053,821.8344,6.9199,2.7232,80,1); //
AddStaticVehicle(411,2052.2217,821.7113,6.9275,7.3336,116,1); //
AddStaticVehicle(411,2049.8057,821.3435,6.9999,1.7290,12,1); //
AddStaticVehicle(411,2047.1245,821.6025,6.9474,359.9328,80,1); //
AddStaticVehicle(411,2044.3140,821.3569,7.0054,0.2944,116,1); //
...
public OnPlayerSpawn(playerid)
{
	...
	{
		SendClientMessage(playerid,COLOR_YELLOW, "You can either register your current player name with '/r [password]'");
		SendClientMessage(playerid,COLOR_YELLOW, "or login using '/l [password]'");
	...
	for(new i=0;i<MAX_PLAYERS;i++)
	{
		if(IsPlayerConnected(i) && IsPlayerInAnyVehicle(i))
		{
			if(speedo[i] == 1 && aMessage[i] == 0 && Count[i] == 0)
			{
	            ...
	            format(string, sizeof(string), "~n~~n~~n~~n~~n~~n~~n~~n~~n~~r~Vehicle: ~g~%s~n~~b~MpH: ~g~%d ~r~/ ~b~Km/H: ~g~%d",carname,floatround(value/2200),floatround(value/1400));
			...
			if(speedo[i] == 2 && aMessage[i] == 0 && Count[i] == 0)
			{
	            ...
	            distance = floatsqroot(floatpower(floatabs(floatsub(x,SavePlayerPos[i][LastX])),2)+floatpower(floatabs(floatsub(y,SavePlayerPos[i][LastY])),2)+floatpower(floatabs(floatsub(z,SavePlayerPos[i][LastZ])),2));
	            value = floatround(distance * 11000);
	            format(string, sizeof(string), "~n~~n~~n~~n~~n~~n~~n~~n~~n~~r~Vehicle: ~g~%s~n~MpH: ~g~%d ~r~/ ~b~Km/H: ~g~%d",carname,floatround(value/2200),floatround(value/1400));
		        GameTextForPlayer(i,string,850,3);
	            SavePlayerPos[i][LastX] = x;
	            SavePlayerPos[i][LastY] = y;
	            SavePlayerPos[i][LastZ] = z;
...
//---------------------------------Login System---------------------------------
	if(strcmp(cmd, "/l", true) == 0)
	{
		...
		{
		    SendClientMessage(playerid, COLOR_BRIGHTRED, "You must spawn before you can use /l or /r");
		    return 1;
		}
		if(logged[playerid] == 1)
		{
	    ...
	    tmp = strtok(cmdtext, idx);
 		...
 		GetPlayerName(playerid, playername, sizeof(playername));
	    if(!strlen(tmp))
			SendClientMessage(playerid, COLOR_LIGHTBLUE, "USAGE: /l [password]");
		...
		else
		{
			if (dini_Exists(udb_encode(playername)))
			{
			    ...
			    format(string, sizeof(string), "The account %s, does not exist on this server. Please type /r [password] to create an account.", playername);
				SendClientMessage(playerid, COLOR_BRIGHTRED, string);
			}
		}
...
//------------------------------Registration System-----------------------------
	if(strcmp(cmd, "/r", true) == 0)
	{
	    ...
	    if(!strlen(tmp))
			SendClientMessage(playerid, COLOR_YELLOW, "USAGE: /r [password]");
...
//----------------------------Admin Stats Search--------------------------------
	if(strcmp(cmd, "/astats", true) == 0)
	{
		...
		if(!strlen(tmp))
		{
			SendClientMessage(playerid, COLOR_LIGHTBLUE, "USAGE: /astats [playerid]");
			return 1;
		}
		if(logged[playerid] == 0)
		{
	...
	if(strcmp(cmd, "/buycar", true) == 0) {
		...
		if(logged[playerid] == 1) {
			...
			if(cash[playerid] >= carcost[playerid]) {
				...
				format(string5, sizeof(string5), "You just bought this %s for $%d.", carname, carcost[playerid],carname,"%");
...
//------------------------------------Credits-----------------------------------
	if(strcmp(cmd, "/credits", true) == 0) {
		...
		SendClientMessage(playerid, COLOR_YELLOW, "Checkpoint system basic design - Project San Andreas - Pixels^");
...
//--------------------------------Commands list---------------------------------
	if (strcmp(cmdtext, "/commands", true)==0)
	{
		...
		SendClientMessage(playerid, COLOR_RED, "You can use the /r and /l commands once you have spawned and /setpass to change your password.");
		...
		SendClientMessage(playerid, COLOR_YELLOW, "To clear the Chat type /clear");
...
//-------------------------------Admin lvl 2 Help-------------------------------
	if(strcmp(cmd, "/ahelp2", true) == 0) {
		...
		SendClientMessage(playerid, COLOR_YELLOW, "Set player admin level - /asetadmin [player id] [admin level] :: (0 - no admin <> 1 - moderator <> 2 - full admin)");
        SendClientMessage(playerid, COLOR_YELLOW, "Get detailed player information - /astats [playerid]");
		return 1;
	}
//--------------------------------Teleport Help---------------------------------
	if (strcmp(cmdtext, "/telehelp", true)==0)
	{
		...
		SendClientMessage(playerid,COLOR_SYSTEM,".:There are two parts for the Teleport help:.");
		SendClientMessage(playerid,COLOR_SYSTEM,"Type /telehelpa for the first part of the teleport help");
		SendClientMessage(playerid,COLOR_SYSTEM,"Type /telehelpb for the second part of the teleport help");
		return 1;
	}
//--------------------------------Teleport Help A-------------------------------
...
//--------------------------------Teleport Help B-------------------------------
	...
	{
		SendClientMessage(playerid,COLOR_SYSTEM,"/worker will teleport you to the WORKER spawnpoint if your on the WORKER team");
		SendClientMessage(playerid,COLOR_SYSTEM,"/pimp will teleport you to the PIMP spawnpoint if your on the PIMP team");
		SendClientMessage(playerid,COLOR_SYSTEM,"/golfer will teleport you to the GOLFER spawnpoint if your on the GOLFER team");
		SendClientMessage(playerid,COLOR_SYSTEM,"/triad will teleport you to the TRIAD spawnpoint if your on the TRIAD team");
		SendClientMessage(playerid,COLOR_SYSTEM,"/army will teleport you to the AMRY spawnpoint if your on the ARMY team");
		SendClientMessage(playerid,COLOR_SYSTEM,"/valet will teleport you to the VALET spawnpoint if your on the VALET team");
		SendClientMessage(playerid,COLOR_SYSTEM,"/medic will teleport you to the MEDIC spawnpoint if your on the MEDIC team");
		SendClientMessage(playerid,COLOR_SYSTEM,"/fbi will teleport you to the FBI spawnpoint if your on the FBI team");
		SendClientMessage(playerid,COLOR_SYSTEM,"/chicken will teleport you to the CHICKEN spawnpoint if your on the CHICKEN team");
		SendClientMessage(playerid,COLOR_SYSTEM,"/rich will teleport you to the RICH spawnpoint if your on the RICH team");
		SendClientMessage(playerid,COLOR_SYSTEM,"/pilot will teleport you to the PILOT spawnpoint if your on the PILOT team");
		SendClientMessage(playerid,COLOR_SYSTEM,"/danang will teleport you to the DANANG spawnpoint if your on the DANANG team");
		return 1;
	}
//-----------------------------------Save pos-----------------------------------
...
public OnPlayerEnterVehicle(playerid, vehicleid, ispassenger)
{
	...
	if(tmp[playerid] == 0)
	{
		...
		if (strcmp(tmp1,server,false) == 0 && logged[playerid] == 0)
		{
			...
			new string[256];
			carcost[playerid] = dini_Int(filename, "carcost");
			format(string, sizeof(string), "This %s is for sale and costs $%d, but you cannot purchase it untill you /l.", carname, carcost[playerid]);
			SendClientMessage(playerid, COLOR_BRIGHTRED, string);
			return 1;
		}
		else
