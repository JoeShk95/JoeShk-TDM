...
SendClientMessage(playerid,COLOR_WHITE,"/sawn join - הצטרפות");
SendClientMessage(playerid,COLOR_WHITE,"/sawn open - התחלה");
SendClientMessage(playerid,COLOR_WHITE,"/kill - פרישה");
return 1;
}

if(strcmp(Roy_Killer,"open",true) == 0)
{
if(SawnOn == 1 || SawnOn == 2) return SendClientMessage(playerid,red,".הקרב סואן כבר החל או שיש כבר קרב סואן פעיל");
format(string,sizeof(string),"[Sawn Champions] .פתח קרב סואן צ'אמפיון %s השחקן",GetName(playerid));
SendClientMessage(playerid,COLOR_LIGHTGREEN,string);
SendClientMessage(playerid,0xF6F658FF,"/Sawn Join - על מנת להצטרף, לקרב סואן");
TogglePlayerControllable(playerid,0);
SetPlayerHealth(playerid,100);
SetPlayerArmour(playerid,100);
GivePlayerWeapon(playerid,26,999999);
SetPlayerPos(playerid,1051.5001,1529.2806,52.4050);
SetPlayerInterior(playerid,0);
playerid = openid;
SawnOn = 1;
InSawn[playerid] = 1;
...
if(strcmp(Roy_Killer,"join",true) == 0)
{
if(SawnOn == 2) return SendClientMessage(playerid,red,".כבר יש סואן פועל עם שחקנים פעילים");
format(string,sizeof(string),"[Sawn Champions] .סגר תקרב סואן, הצטרף %s השחקן",GetName(playerid));
SendClientMessage(playerid,COLOR_LIGHTGREEN,string);
TogglePlayerControllable(playerid,1);
TogglePlayerControllable(openid,1);
SetPlayerPos(playerid,1146.8010,1529.4210,52.4155);
GivePlayerWeapon(playerid,26,999999);
SetPlayerHealth(playerid,100);
SetPlayerArmour(playerid,100);
SawnOn = 2;
...
if(killerid == INVALID_PLAYER_ID)
{
format(sts, sizeof(sts), ".פרש מהקרב סואן ,'%s' השחקן",GetName(playerid));
SendClientMessageToAll(COLOR_KHAKI, sts);
LACY_GiveMoney(killerid,3000);
InSawn[playerid] = 0;
InSawn[killerid] = 0;
openid = - 1;
SawnOn = 0;
...
format(sts, sizeof(sts), ".בקרב סואן אחד על אחד %s ניצח את %s השחקן",GetName(killerid),GetName(killerid));
SendClientMessageToAll(COLOR_KHAKI, sts);
LACY_GiveMoney(killerid,3000);
InSawn[playerid] = 0;
InSawn[killerid] = 0;
