...
new Objects[MAX_STREAM_OBJECTS][object_enum],PlayerObjects[MAX_PLAYERS][player_enum],created;
...
public Core_DestroyStreamObject(id2)
{
	...
	{
	    if (IsPlayerConnected(i) && GetPlayerState(i) != 7)
	    {
...
public Core_SetStreamObjectPos(id2,Float:xpos2,Float:ypos2,Float:zpos2)
{
    ...
    for (new i=0; i<MAX_PLAYERS; i++)
    {
    if (IsPlayerConnected(i) && GetPlayerState(i) != 7 && PlayerObjects[i][seen][id2] == true) SetPlayerObjectPos(i,PlayerObjects[i][pobjects][id2],xpos2,ypos2,zpos2);
	}
}
...
public Core_SetStreamObjectRot(id2,Float:xrot2,Float:yrot2,Float:zrot2)
{
    ...
    for (new i=0; i<MAX_PLAYERS; i++)
    {
	if (IsPlayerConnected(i) && GetPlayerState(i) != 7 && PlayerObjects[i][seen][id2] == true) SetPlayerObjectRot(i,PlayerObjects[i][pobjects][id2],xrot2,yrot2,zrot2);
	}
}
...
public Core_AttachStreamObjectToPlayer(id2,playerid,Float:xoff2,Float:yoff2,Float:zoff2,Float:xrot2,Float:yrot2,Float:zrot2)
{
    ...
    for (new i=0; i<MAX_PLAYERS; i++)
    {
	if (IsPlayerConnected(i) && GetPlayerState(i) != 7 && PlayerObjects[i][seen][id2] == true) AttachPlayerObjectToPlayer(i,PlayerObjects[i][pobjects][id2],playerid,xoff2,yoff2,zoff2,xrot2,yrot2,zrot2);
	}
}
...
public Core_MoveStreamObject(id2,Float:movx2,Float:movy2,Float:movz2,Float:speed2)
{
    ...
    for (new i=0; i<MAX_PLAYERS; i++)
    {
	if (IsPlayerConnected(i) && GetPlayerState(i) != 7 && PlayerObjects[i][seen][id2] == true) MovePlayerObject(i,PlayerObjects[i][pobjects][id2],movx2,movy2,movz2,speed2);
	}
	...
	bool:yisbigger = (movy2 >= Objects[id2][ypos]) ? true : false,
	bool:zisbigger = (movz2 >= Objects[id2][zpos]) ? true : false;
	...
	yadd = (time == 0.0) ? 0 : (yisbigger) ? floatround(((movy2 - Objects[id2][ypos])/time),floatround_ceil) : floatround(((movy2 - Objects[id2][ypos])/time),floatround_floor),
	zadd = (time == 0.0) ? 0 : (zisbigger) ? floatround(((movz2 - Objects[id2][zpos])/time),floatround_ceil) : floatround(((movz2 - Objects[id2][zpos])/time),floatround_floor);
	KillTimer(Objects[id2][movetimer]);
...
public MoveTimer(id2,xadd,yadd,zadd,bool:xisbigger,bool:yisbigger,bool:zisbigger)
{
	...
	if (reached)
	{
	    ...
	    for (new i=0; i<MAX_PLAYERS; i++)
	    {
              if (IsPlayerConnected(i) && GetPlayerState(i) != 7 && PlayerObjects[i][seen][id2] == true && Objects[id2][moving] == true) MovePlayerObject(i,PlayerObjects[i][pobjects][id2],Objects[id2][movx],Objects[id2][movy],Objects[id2][movz],Objects[id2][speed]);
        }
